...
|---------|-------|-------------|
| extensions.rockbox_coverart.art_name_on_device  | cover.jpg | Only used if convert_and_resize_art=False |
| extensions.rockbox_coverart.art_name_on_device_noext | cover | Desired filename, without extension, on device. Used if convert_and_resize_art=True. |
| extensions.rockbox_coverart.convert_and_resize_art | **True**/False | Enable conversion and resize operations |
| extensions.rockbox_coverart.convert_filetype | jpeg | Desired filetype string - jpg will be autocorrected to jpeg. png also works, but rockbox cannot use png as coverart. Only used if convert_and_resize_art=True. |
